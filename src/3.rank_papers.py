#!/usr/bin/env python
# 使用 Rerank API 对候选论文做重排序（支持 BLT 和硅基流动）。
...
from llm import BltClient, SiliconflowClient, parse_provider_model

SCRIPT_DIR = os.path.dirname(__file__)
ROOT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))
TODAY_STR = str(os.getenv("DPR_RUN_DATE") or "").strip() or datetime.now(
    timezone.utc
).strftime("%Y%m%d")
...
def build_documents(
    papers_by_id: Dict[str, Dict[str, Any]], paper_ids: List[str]
) -> List[str]:
    docs: List[str] = []
...
def get_top_ids(query_obj: Dict[str, Any]) -> List[str]:
    ...
    if not top_ids and isinstance(sim_scores, dict) and sim_scores:
        top_ids = sorted(
            sim_scores.keys(), key=lambda pid: sim_scores[pid].get("rank", 1e9)
        )
    return list(top_ids)
    ...
    *,
    ...
    for q in queries or []:
        ...
        for rank_idx, pid in enumerate(top_ids, start=1):
            ...
            if not paper_id:
                continue
            score_map[paper_id] = score_map.get(paper_id, 0.0) + 1.0 / (
                RRF_K + rank_idx
            )
...
def process_file(
    ...
    if not papers_list or not all_queries:
        log(
            f"[WARN] 文件 {os.path.basename(input_path)} 中缺少 papers 或 queries，跳过。"
        )
        return
...
def main() -> None:
    ...
    if not os.path.exists(input_path):
        log(
            f"[WARN] 输入文件不存在（今天可能没有新论文）：{input_path}，将跳过 Step 3。"
        )
        return

    rerank_provider = (
        args.rerank_provider or os.getenv("RERANK_PROVIDER", "blt").strip().lower()
    )
    ...
    if not rerank_model:
        if rerank_provider == "siliconflow":
            rerank_model = (
                os.getenv("SILICONFLOW_RERANK_MODEL") or "BAAI/bge-reranker-v2-m3"
            )
        else:
            rerank_model = (
                os.getenv("BLT_RERANK_MODEL")
    ...
    if rerank_provider == "siliconflow":
        ...
        if not api_key:
            raise RuntimeError(
                "缺少 SILICONFLOW_API_KEY 环境变量，无法调用硅基流动 Rerank API。"
            )
