...
class RemoteSentenceTransformer:
    ...
    def _get_local_model(self):
        if self._local_model is None:
            ...
            if self.max_seq_length is not None and hasattr(
                self._local_model, "max_seq_length"
            ):
                try:
        ...
        try:
            ...
            for chunk_index, chunk in enumerate(chunks, start=1):
                ...
                if response.status_code == 401 and headers.get("Authorization"):
                    self._log(
                        "[WARN] 远程 embedding 鉴权失败，自动回退为无鉴权请求重试一次。"
                    )
                    headers = {
                        "Content-Type": "application/json",
                    }
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"远程 embedding 返回无法转换为 float32：{exc}"
                    ) from exc

                if arr.ndim != 2:
                    raise RuntimeError(
                        f"远程 embedding 返回维度异常：shape={getattr(arr, 'shape', None)}"
                    )
            ...
            merged = (
                np.vstack(outputs) if outputs else np.zeros((0, 0), dtype=np.float32)
            )
        ...
        try:
            ...
            for chunk_index, chunk in enumerate(chunks, start=1):
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"硅基流动 embedding 返回无法转换为 float32：{exc}"
                    ) from exc

                if arr.ndim != 2:
                    raise RuntimeError(
                        f"硅基流动 embedding 返回维度异常：shape={getattr(arr, 'shape', None)}"
                    )
    ...
    *,
    ...
    if embed_provider == "siliconflow":
        ...
        siliconflow_endpoint = os.getenv(
            "SILICONFLOW_EMBED_ENDPOINT", _DEFAULT_SILICONFLOW_EMBED_ENDPOINT
        ).strip()
        remote_timeout_text = os.getenv(
            "DPR_EMBED_API_TIMEOUT", str(_DEFAULT_REMOTE_TIMEOUT_SECONDS)
        )
        try:
    ...
    if remote_endpoint and not allow_remote:
        log(
            f"[INFO] 已禁用远程 embedding，强制使用本地模型：{model_name} (device={device})"
        )
    ...
    if retries is None:
        ...
        if env_retries is None:
            ...
            try:
                ...
                print(
                    f"[WARN] 环境变量 LLM_EMBED_MODEL_RETRIES 无效：{env_retries}，回退默认 {_DEFAULT_RETRIES}"
                )
                retries = _DEFAULT_RETRIES
        ...
        for provider_name, endpoint in providers:
            try:
                ...
                with (
                    ...
                    _hf_http_backoff(max_retries=hf_backoff_retries),
                ):
                    from sentence_transformers import SentenceTransformer
