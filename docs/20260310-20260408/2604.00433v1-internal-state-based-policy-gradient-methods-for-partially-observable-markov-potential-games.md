...
motivation: 旨在解决部分可观测马尔可夫势博弈中由于信息不对称和历史信息随时间无限增长导致的决策难题。
method: 结合公共信息框架与有限状态控制器来压缩内部状态，并采用自然策略梯度算法寻找博弈的纳什均衡。
result: 建立了非渐近收敛界，并证明了该方法在多个部分可观测环境下的性能显著优于仅使用当前观测的策略。
conclusion: 该研究为处理具有复杂信息结构的多智能体强化学习问题提供了一种具有理论保障且计算可行的有效方案。
---

## 摘要
本文研究了部分可观测马尔可夫势博弈中的多智能体强化学习。由于部分可观测性、分散信息以及维数灾难，解决这一问题具有挑战性。首先，为了应对前两个挑战，我们利用了公共信息框架，该框架允许智能体基于共享信息和局部信息进行决策。其次，为了确保可处理性，我们研究了一种压缩累积信息的内部状态，防止其随时间无限增长。然后，我们实现了一种基于内部状态的自然策略梯度方法，以寻找马尔可夫势博弈的纳什均衡。我们的主要贡献是为该方法建立了一个非渐近收敛界。我们的理论界限可分解为两个可解释的部分：一个是在标准马尔可夫势博弈中也会出现的统计误差项，以及一个捕捉有限状态控制器使用的近似误差项。最后，在多个部分可观测环境下的仿真表明，与仅使用当前观测的设置相比，所提出的使用有限状态控制器的方法在性能上取得了持续的提升。
