...
tags: ["query:rl-agt"]
score: 7.0
evidence: 用于机器人命令理解的语法约束大语言模型
tldr: 针对工业人机协作中大语言模型指令生成不够精确的问题，本文提出一种语法约束的LLM框架。该方法结合了微调LLM的上下文推理能力与语法驱动的自然语言理解系统，通过两阶段处理将自然语言转化为标准化的机器人JSON指令。系统引入了自动纠错的验证反馈机制，显著提升了指令的有效性与安全性，为工业环境下的可靠通信提供了新方案。
source: arxiv
...
motivation: 工业机器人需要极高的指令精确度，而通用大语言模型在生成可执行指令时往往缺乏必要的领域约束和确定性。
method: 采用微调LLM进行高层推理，并结合结构化语言模型与语法解析器，通过验证反馈循环强制输出符合预定义规则的JSON格式指令。
result: 在HuRIC数据集上的评估表明，该混合模型在指令有效性方面显著优于纯微调LLM和传统的语法驱动模型。
conclusion: 结合语法约束与大语言模型能有效提升机器人指令理解的鲁棒性与安全性，是实现高效工业人机协作的关键技术。
---

## 摘要
工业环境中的人机协作需要精确且可靠的通信，以提高运营效率。虽然大语言模型（LLMs）能够理解通用语言，但它们往往缺乏安全且可执行的工业指令所需的领域特定严谨性。为了填补这一空白，本文介绍了一种新型的语法约束 LLM，它将语法驱动的自然语言理解（NLU）系统与微调后的 LLM 相结合，从而兼顾了对话的灵活性和机器人技术所需的确定性精度。我们的方法采用了一个两阶段过程。首先，微调后的 LLM 对自然语言输入进行高级上下文推理和参数推断。其次，结构化语言模型（SLM）和基于语法的规范化器对 LLM 的输出进行约束，强制其转换为由有效动作帧和指令元素组成的标准化符号格式。该过程保证了生成的指令是有效的，并以机器人可读的 JSON 格式进行结构化。所提模型的一个关键特征是验证与反馈循环。语法解析器根据预定义的机器人可执行动作列表对输出进行验证。如果指令无效，系统会自动生成纠正性提示并重新调用 LLM。这种迭代自纠错机制使模型能够从初始解释错误中恢复，从而提高系统的鲁棒性。我们在 Human Robot Interaction Corpus (HuRIC) 数据集上，将我们的语法约束混合模型与两个基准模型（基于 API 的微调 LLM 和独立的语法驱动 NLU 模型）进行了评估。结果表明，该混合方法实现了卓越的指令有效性，促进了更安全、更高效的工业人机协作。
