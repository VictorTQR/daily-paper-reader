...
tags: ["query:rl-agt"]
score: 8.0
evidence: 基于大语言模型智能体的隐私驱动遗忘框架
tldr: 针对大语言模型（LLM）智能体在实际应用中积累敏感或过时知识的隐私风险，本文首次提出了LLM智能体遗忘（Unlearning）框架。该框架将遗忘场景细分为状态、轨迹和环境三类，并引入一种基于自然语言的方法，通过转换模型将遗忘请求转化为引导智能体受控遗忘的提示词。此外，研究还设计了推理对手来评估遗忘的鲁棒性。实验表明，该方法在有效擦除特定知识的同时，能保持智能体在其他任务上的性能。
source: arxiv
...
motivation: LLM智能体在集成到现实应用时会积累敏感或过时信息，亟需一种机制实现选择性遗忘以保护隐私和确保知识准确性。
method: 提出了一个包含状态、轨迹和环境遗忘的综合框架，并利用转换模型将高层自然语言指令转化为可执行的遗忘提示词。
result: 实验证明该方法能有效引导智能体遗忘目标知识并抵御推理攻击，同时不损害其在非目标任务上的原有性能。
conclusion: 本研究为LLM智能体的隐私驱动遗忘奠定了基础，提供了一套从场景分类到方法实现及安全性评估的完整方案。
---

## 摘要
基于大语言模型（LLM）的智能体因其强大的推理能力，近期受到了广泛关注。现有研究主要集中在提升这些智能体在不同场景下的任务性能。然而，随着基于 LLM 的智能体越来越多地集成到现实应用中，关于其积累敏感或过时知识的担忧日益凸显。解决这些问题需要开发允许智能体选择性遗忘先前学习知识的机制，从而产生了一个新术语：基于 LLM 智能体的机器遗忘（unlearning）。本文开启了对基于 LLM 智能体机器遗忘的研究。具体而言，我们提出了一个新颖且全面的框架，将遗忘场景分为三种语境：状态遗忘（遗忘特定状态或项目）、轨迹遗忘（遗忘动作序列）和环境遗忘（遗忘整个环境或任务类别）。在该框架内，我们引入了一种基于自然语言的遗忘方法，通过训练转换模型将高层遗忘请求转化为可执行的遗忘提示词，引导智能体完成受控的遗忘过程。此外，为了评估所提框架的鲁棒性，我们引入了一个遗忘推理对手，该对手能够构建提示词、查询智能体并观察其行为，试图推断出已遗忘的知识。实验结果表明，我们的方法能有效使智能体遗忘目标知识，同时保持在非目标任务上的性能，并防止对手推断出已遗忘的知识。
